...
def log_prior(theta):
    ...
    if 0 < e < 2 * np.pi and 0 <= f <= np.pi:  # e in range [0, 2pi] and f in [0, pi]
        prior_e = np.sin(e)+1e-6  # Prior for e
        prior_f = np.sin(f)+1e-6  # Prior for f
...
def run_mcmc(n_walkers, n_iterations, data):
    # Initialize walkers with a larger spread, ensuring they are sufficiently independent
    initial_guess = np.array([d_true, e_true, f_true])  # Use true parameter values as a starting point
    spread_factor = 1.05  # Spread factor to ensure walkers are sufficiently diverse

    # Initialize walkers by adding random noise to the true parameter values
    pos = initial_guess + np.random.randn(n_walkers, 3) * spread_factor

    # Check if the number of walkers is less than 10, and adjust accordingly
    if n_walkers < 10:
        print(f"Warning: Using {n_walkers} walker(s). Consider increasing the number of walkers for better performance.")

    # Set up the sampler using emcee, explicitly using only the random walk move (DEMove)
    ...
    # Explicitly set the move to StretchMove
    sampler._move = emcee.moves.StretchMove()  # Assign StretchMove directly
    ...
    for i in range(nsteps):
        ...
        walkers_pos= all_walkers_positions[:, -1, :].reshape(-1, 3)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f"{n_walkers} walker{'s' if n_walkers != 1 else ''}, {n_iterations} iterations_correlation.png")
...
plt.tight_layout()
plt.savefig(f"{n_walkers} walker{'s' if n_walkers != 1 else ''}, {n_iterations} iterations_final_positions.png")
